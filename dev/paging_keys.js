...
/**
 * A modified release of the Paging Keys javascript library for compatibility with the Invision Power Suite platform
 * @author	Matthew Hutchinson (https://github.com/matthutchinson/paging_keys_js)
 * @author	Makoto Fujimoto (https://www.Makoto.io)
 * @license	Attribution 2.0 UK: England & Wales
 */
var pagingKeys = function() {

	// settings
	var config = {
		nodeSelector:        'article.ipsComment',  // used to select each item on the page and place in the map (must be a link)
		prevPageSelector:    'li.ipsPagination_prev > a:last',                // link on this element should always jump to prev page a.prev_page (must be a link)
		nextPageSelector:    'li.ipsPagination_next > a:last',                // link on this element should always jump to next page a.next_page (must be a link)
		...
		keyPrevPage:         'l',
		keyRefresh:          'r',
		currentItemClass:    'ipsComment_current',        // optional, class assigned to each element you jump to, set to null or remove to disable
		additionalBodyClass: 'paging-keys',               // this class is assigned to the page body on load
		bottomAnchor:        'replyForm'                     // the name of the anchor (without #) at end of page, e.g. set on last post on the page
	};
	...
	function init() {
		...
		// Disable hot-keys as soon as CKEditor has been loaded
		$(window).load(function(e) {
			config['keyNext'] = ips.getSetting('kbnav_keyNext');
			config['keyPrev'] = ips.getSetting('kbnav_keyPrev');
			config['keyNextPage'] = ips.getSetting('kbnav_keyNextPage');
	...
	function initHotKeys() {
		try {
			...
			console.log('Oops, paging_keys requires HotKeys.js (http://la.ma.la/blog/diary_200511041713.htm)');
			console.log(e);
		}
		if (hot_key) {
			if ( config.keyNext ) {
	...
	function disableHotKeys() {
		if (hot_key) {
			...
			hot_key.remove(config.keyRefresh);
		}
	}
	...
	function movePageNext() {
		if (getEl(config.nextPageSelector)) {
			if (getEl(config.nextPageSelector).attr('href') != null) {
				var lastPage = getEl('.ipsPagination').data('ipspagination-pages')
				if ( !lastPage ) {
					lastPage = getEl('.ipsPagination').data('pages')
				}

				if (getEl(config.nextPageSelector).data('page') <= lastPage) {
					redirect(getEl(config.nextPageSelector).attr('href'));
					disableHotKeys();
					return true;
				}
			}
	...
	function movePagePrev() {
		if (getEl(config.prevPageSelector)) {
			if (getEl(config.prevPageSelector).attr('href') != null) {
				if (getEl(config.prevPageSelector).data('page')) {
					redirect(getEl(config.prevPageSelector).attr('href') + '#' + config.bottomAnchor);
					disableHotKeys();
					return true;
				}
			}
